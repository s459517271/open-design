...
- **`version_metadata_url` is the authoritative "did a package ship?" signal, not the workflow conclusion.** The conclusion goes red for anything anywhere in the release run, including work that happens after publication, so keying a notification off it paints the card red while its own download buttons work. `tools/release/src/notifications/feishu.ts` reads `VERSION_METADATA_URL` for both header colour and smoke annotations, falling back to `BUILD_STATE` only for callers that do not pass it.
- The one-shot webhook card in `notify-release-feishu.yml`'s `notify` job is transitional and runs alongside the progressive card until the new one has carried a couple of real releases; a gap in release notification is worse than a duplicate. Retiring it means deleting that job and the `FEISHU_RELEASE_WEBHOOK` / `FEISHU_RELEASE_SIGN_SECRET` secrets it reads.
